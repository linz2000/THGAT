...
class AttnModel(torch.nn.Module):
    """Attention based temporal heterogeneous layers
    """
    def __init__(self, feat_dim, edge_dim, time_dim,
                 sig_dim,
        ...
        args:
          ...
          sig_dim: dim for the node signatures
        ...
        self.edge_in_dim = (feat_dim + edge_dim + time_dim + sig_dim)
    ...
    def forward(self, src, src_t, src_s, seq, seq_t, seq_e, seq_s, mask):
        """"Attention based temporal attention forward pass
        args:
          src: float Tensor of shape [B, D]
          src_t: float Tensor of shape [B, Dt], Dt == D
          src_s: float Tensor of shape [B, Ds], Ds == D
          seq_s: float Tensor of shape [B, N, Ds], Ds == D
        ...
        src_sig = torch.unsqueeze(src_s, dim=1) # src_s [B, 1, D]
        q = torch.cat([src_ext, src_e_ph, src_t, src_sig], dim=2) # [B, 1, D + De + Dt + Ds] -> [B, 1, D]
        k = torch.cat([seq, seq_e, seq_t, seq_s], dim=2) # [B, N, D + De + Dt + Ds] -> [B, N, D]

        mask = torch.unsqueeze(mask, dim=2) # mask [B, N, 1]
        mask = mask.permute([0, 2, 1]) #mask [B, 1, N]

        # # target-attention
        output, attn = self.multi_head_target(q=q, k=k, v=k, mask=mask) # output: [B, 1, D + De + Dt + Ds], attn: [B, 1, N]
...
class THGAN(torch.nn.Module):
    def __init__(self, ngh_finder, n_feat, e_feat, sig_feat,
        ...
        self.sig_feat_th = torch.nn.Parameter(torch.from_numpy(sig_feat.astype(np.float32)))
        self.sig_raw_embed = torch.nn.Embedding.from_pretrained(self.sig_feat_th, padding_idx=0, freeze=True)
        self.sig_dim = sig_feat.shape[1]
    ...
    def tem_conv(self, src_idx_l, cut_time_l, curr_layers, num_neighbors=20):
        ...
        src_node_sig_feat = self.sig_raw_embed(src_node_batch_th)
        ...
        if curr_layers == 0:
            ...
            # sig features
            # src_node_sig_feat = self.sig_raw_embed(src_idx_l)
            src_ngn_sig_feat = self.sig_raw_embed(src_ngh_node_batch_th)
            # sig feature need to be standarized
            # src_node_sig_feat = torch.nn.functional.normalize(src_node_sig_batch, dim=1)
            # src_ngn_sig_feat = torch.nn.functional.normalize(src_ngh_sig_batch, dim=1)
            # src_node_sig_feat = torch.from_numpy(src_node_sig_batch).float().to(device)
            # src_ngn_sig_feat = src_ngh_sig_batch
